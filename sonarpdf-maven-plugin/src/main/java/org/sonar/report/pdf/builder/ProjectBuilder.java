...
public class ProjectBuilder {
	...
	private void initMostViolatedRules(Project project) throws IOException, ReportException {
		...
		for (int i = priorities.length - 1; i >= 0 && limit > 0; i--) {
			...
			if (mostViolatedRulesByLevel != null) {
				...
				Logger.debug("\t " + count + " " + priorities[i]
						+ " violations");
				limit = limit - count;
			} else {
				Logger.debug("There is not result on select //resources/resource");
				Logger.debug("There are no violations with level "
						+ priorities[i]);
			}
		}
	}
	...
	private void initMostViolatedFiles(Project project) throws IOException {
		...
		ResourceQuery resourceQuery = ResourceQuery.createForMetrics(project.getKey(),
		...
		List<FileInfo> fileInfoList = FileInfoBuilder.initFromDocument(
				resources, FileInfo.VIOLATIONS_CONTENT);
	...
	private int initMostViolatedRulesFromNode(Project project, final Resource mostViolatedNode)
