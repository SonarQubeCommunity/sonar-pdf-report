...
public class DefaultPDFReporter extends PDFReporter {
	...
	public DefaultPDFReporter(Credentials credentials, URL logo,
			String projectKey, Properties configProperties,
	...
	private void printDashboard(Project project, Section section)
		...
		Font dataFont2 = new Font(Font.TIMES_ROMAN, 10, Font.BOLD, new Color(
				100, 150, 190));

		PdfPTable linesOfCode = new PdfPTable(1);
		linesOfCode.getDefaultCell().setBorderColor(Color.WHITE);
		linesOfCode.addCell(new Phrase(
				getTextProperty("general.lines_of_code"), titleFont));
		linesOfCode.addCell(new Phrase(project.getMeasure("ncss")
		...
		linesOfCode.addCell(new Phrase(project.getMeasure(
				"duplicated_lines_ratio").getFormatValue()
				+ " duplicated lines", dataFont2));
		...
		complexity.addCell(new Phrase(project.getMeasure("ccn")
				.getFormatValue() + " decision points", dataFont2));
		...
		rulesCompliance.addCell(new Phrase(project.getMeasure(
				"rules_compliance").getFormatValue(), dataFont));
	...
	private void printMeasures(Measures measures, Section section) {
		...
		versioningTable.addCell(new Phrase(super
				.getTextProperty("general.versioning_information"),
				Style.TITLE_FONT));
		...
		measuresTable.addCell(new Phrase(super
				.getTextProperty("general.metric"), Style.TITLE_FONT));
		measuresTable.addCell(new Phrase(
				super.getTextProperty("general.value"), Style.TITLE_FONT));
		...
		while (it.hasNext()) {
			...
			if (!measureKey.equals("ccn_classes_count_distribution")
				...
				measuresTable.getDefaultCell().setHorizontalAlignment(
						PdfCell.ALIGN_LEFT);
				measuresTable.addCell(super.getTextProperty("metrics."
						+ measureKey));
				measuresTable.getDefaultCell().setHorizontalAlignment(
	...
	private void printProjectInfo(Project project, Section section)
		...
		if (project.getSubprojects().size() != 0) {
			...
			sublist.add(new ListItem(super
					.getTextProperty("general.no_modules")));
		}
	...
	protected void printTocTitle(Toc tocDocument) throws DocumentException {
		Paragraph tocTitle = new Paragraph(
				super.getTextProperty("main.table.of.contents"),
	...
	protected void printFrontPage(Document frontPageDocument,
		...
		try {
			URL largeLogo;
			if (super.getConfigProperty("front.page.logo")
					.startsWith("http://")) {
			...
			float positionX = pageSize.getWidth() / 2f - logoImage.getWidth()
					/ 2f;
			logoImage.setAbsolutePosition(positionX, pageSize.getHeight()
					- logoImage.getHeight() - 100);
			frontPageDocument.add(logoImage);

			PdfPTable title = new PdfPTable(1);
			title.getDefaultCell()
			...
			String dateRow = df.format(super.getProject().getMeasures()
					.getDate());
			...
			title.setTotalWidth(pageSize.getWidth()
